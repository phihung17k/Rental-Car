 ...
 * @author Win 10
 */
@WebFilter(filterName = "DeleteCarInCartFilter", urlPatterns = {"/delete"})
public class DeleteCarInCartFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            ...
            if (stringRentalDate.matches(pattern) && stringReturnDate.matches(pattern)) {
                SimpleDateFormat simpleDate = new SimpleDateFormat("yyyy-MM-dd");
                Date rentalDate = simpleDate.parse(stringRentalDate);
                Date returnDate = simpleDate.parse(stringReturnDate);
                if (rentalDate.compareTo(returnDate) < 1) {
                    url = DELETE_CAR_IN_CART_CONTROLLER;
                }
            }
            if (url != null) {
    ...
    public void init(FilterConfig filterConfig) {
        ...
        if (filterConfig != null) {
            if (debug) {
                log("DeleteCarInCartFilter:Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        if (filterConfig == null) {
            return ("DeleteCarInCartFilter()");
        }
        StringBuffer sb = new StringBuffer("DeleteCarInCartFilter(");
